...
- [x] **Artwork candidate chosen (14/09/2026):** under-bridge photo, band standing among converging industrial steel beams, direct-to-camera gaze (not the saluting variant from the same setup — too gimmicky for a static cover, save that one for Reels/BTS instead). Riverbank/rocks photo set aside as secondary — usable for teaser/B-roll, not the cover.
- [ ] **OPEN — still needed:** proper 3000x3000 square crop of the chosen photo. Source is portrait orientation — crop centered on the vocalist, keep the converging beams visible above for depth, sacrifice leg/boot framing at the bottom if needed. Do this with a real editor, not an auto-crop. Once cropped, swap it in on DistroKid (free/instant pre-release) to replace the current monochrome placeholder — resolve before 16/10.
